...
import { readableStreamToString, SoundCloudError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new SoundCloudError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new SoundCloudError(e, e.response?.data)
                }
                if (innerError instanceof SoundCloudError) {
                    // Got "SoundCloudError" from the above try block
                    throw innerError;
                }
